defmodule SonosexTest do
  ...
  describe "Speaker" do
    test "coordinator?/1 returns true when uuid matches coordinator_uuid" do
      ...
      assert Sonosex.Speaker.coordinator?(speaker)
    end
    ...
    test "coordinator?/1 returns false when uuid differs from coordinator_uuid" do
      ...
      refute Sonosex.Speaker.coordinator?(speaker)
    end
  ...
  describe "SOAP.build_envelope/3" do
    test "builds valid SOAP XML envelope" do
      ...
      envelope = Sonosex.SOAP.build_envelope(urn, "Play", InstanceID: 0, Speed: 1)
    ...
    test "builds envelope with no params" do
      ...
      envelope = Sonosex.SOAP.build_envelope(urn, "GetZoneGroupState", [])
    ...
    test "escapes XML special characters in param values" do
      urn = "urn:schemas-upnp-org:service:AVTransport:1"
      envelope = Sonosex.SOAP.build_envelope(urn, "SetAVTransportURI", CurrentURI: "http://example.com?a=1&b=2")
  ...
  describe "XML.parse/1" do
    test "parses simple XML" do
      doc = Sonosex.XML.parse("<root><child>hello</child></root>")
      assert is_tuple(doc)
    end

    test "parses XML with attributes" do
      doc = Sonosex.XML.parse(~s(<root attr="value"><child>text</child></root>))
      assert is_tuple(doc)
    end
  end
  ...
  describe "XML.children_to_map/1" do
    test "extracts child elements as string map" do
      doc = Sonosex.XML.parse("<root><Name>Living Room</Name><Volume>42</Volume></root>")
      map = Sonosex.XML.children_to_map(doc)
    ...
    test "handles empty children" do
      doc = Sonosex.XML.parse("<root></root>")
      map = Sonosex.XML.children_to_map(doc)
  ...
  describe "XML.get_attribute/2" do
    test "extracts attribute value" do
      doc = Sonosex.XML.parse(~s(<zone UUID="RINCON_123" ZoneName="Kitchen"/>))
      assert Sonosex.XML.get_attribute(doc, :UUID) == "RINCON_123"
      assert Sonosex.XML.get_attribute(doc, :ZoneName) == "Kitchen"
    end

    test "returns nil for missing attribute" do
      doc = Sonosex.XML.parse(~s(<zone UUID="RINCON_123"/>))
      assert Sonosex.XML.get_attribute(doc, :Missing) == nil
    end
  end

  describe "XML.get_elements/2" do
    test "finds child elements by tag name" do
      doc = Sonosex.XML.parse("<root><item>a</item><item>b</item><other>c</other></root>")
      items = Sonosex.XML.get_elements(doc, :item)

      assert length(items) == 2
    end

    test "returns empty list when no matches" do
      doc = Sonosex.XML.parse("<root><other>x</other></root>")
      assert Sonosex.XML.get_elements(doc, :item) == []
    end
  end
  ...
  describe "XML.unescape_and_parse/1" do
    test "unescapes HTML entities and parses" do
      ...
      doc = Sonosex.XML.unescape_and_parse(encoded)
